...
use libc::{c_int, fflush, fgets, fprintf, strcmp};
...
unsafe extern "C" fn getthread(L: *mut lua_State, arg: *mut libc::c_int) -> *mut lua_State {
    if lua_isthread(L, 1) {
        *arg = 1 as libc::c_int;
...
unsafe extern "C" fn checkstack(L: *mut lua_State, L1: *mut lua_State, n: libc::c_int) {
    if L != L1 && lua_checkstack(L1, n) == 0 {
        luaL_error(
            L,
            cstr!("stack overflow") as *const u8 as *const libc::c_char,
        );
    }
}

unsafe extern "C" fn db_debug(L: *mut lua_State) -> libc::c_int {
    loop {
        let mut buffer: [libc::c_char; 250] = [0; 250];
        if (fgets(
            buffer.as_mut_ptr(),
            ::core::mem::size_of::<[libc::c_char; 250]>() as libc::c_ulong as libc::c_int,
...
unsafe extern "C" fn db_traceback(L: *mut lua_State) -> libc::c_int {
...
unsafe extern "C" fn db_getuservalue(L: *mut lua_State) -> libc::c_int {
...
unsafe extern "C" fn db_setuservalue(L: *mut lua_State) -> libc::c_int {
...
unsafe extern "C" fn db_setlocal(L: *mut lua_State) -> libc::c_int {
    let mut arg: libc::c_int = 0;
    ...
    let level = luaL_checkinteger(L, arg + 1 as libc::c_int) as libc::c_int;
    let nvar = luaL_checkinteger(L, arg + 2 as libc::c_int) as libc::c_int;
    if lua_getstack(L1, level, &mut ar) == 0 {
        return luaL_argerror(L, arg + 1 as libc::c_int, cstr!("level out of range"));
    }
...
unsafe extern "C" fn db_getlocal(L: *mut lua_State) -> libc::c_int {
    let mut arg: libc::c_int = 0;
    let L1 = getthread(L, &mut arg);
    let mut ar = lua_Debug {
        event: 0,
...
unsafe extern "C" fn db_getregistry(L: *mut lua_State) -> libc::c_int {
    lua_pushvalue(L, LUA_REGISTRYINDEX);
    return 1 as libc::c_int;
}

unsafe extern "C" fn db_setmetatable(L: *mut lua_State) -> libc::c_int {
    let t = lua_type(L, 2 as libc::c_int);
...
unsafe extern "C" fn db_getmetatable(L: *mut lua_State) -> libc::c_int {
...
unsafe extern "C" fn auxupvalue(L: *mut lua_State, get: libc::c_int) -> libc::c_int {
    let name: *const libc::c_char;
    let n = luaL_checkinteger(L, 2 as libc::c_int) as libc::c_int;
    ...
    lua_pushstring(L, name);
    lua_insert(L, -(get + 1 as libc::c_int));
    return get + 1 as libc::c_int;
}

unsafe extern "C" fn db_getupvalue(L: *mut lua_State) -> libc::c_int {
    return auxupvalue(L, 1 as libc::c_int);
}

unsafe extern "C" fn db_setupvalue(L: *mut lua_State) -> libc::c_int {
...
unsafe extern "C" fn checkupval(
    ...
    let nup = luaL_checkinteger(L, argnup) as libc::c_int;
...
unsafe extern "C" fn db_upvalueid(L: *mut lua_State) -> libc::c_int {
    let n = checkupval(L, 1 as libc::c_int, 2 as libc::c_int);
    lua_pushlightuserdata(L, lua_upvalueid(L, 1 as libc::c_int, n));
    return 1 as libc::c_int;
}

unsafe extern "C" fn db_upvaluejoin(L: *mut lua_State) -> libc::c_int {
    let n1 = checkupval(L, 1 as libc::c_int, 2 as libc::c_int);
    let n2 = checkupval(L, 3 as libc::c_int, 4 as libc::c_int);
    ...
    [
        {
            let init = luaL_Reg {
                ...
                func: Some(db_debug as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_getuservalue as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_gethook as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_getinfo as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_getlocal as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_getregistry as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_getmetatable as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_getupvalue as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_upvaluejoin as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_upvalueid as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_setuservalue as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_sethook as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_setlocal as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_setmetatable as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_setupvalue as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
            let init = luaL_Reg {
                ...
                func: Some(db_traceback as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            };
            init
        },
        {
...
#[no_mangle]
pub unsafe extern "C" fn luaopen_debug(L: *mut lua_State) -> libc::c_int {
    luaL_newlib(L, dblib.as_ptr());
    return 1 as libc::c_int;
}

extern "C" {
    fn db_gethook(L: *mut lua_State) -> libc::c_int;
    fn db_getinfo(L: *mut lua_State) -> libc::c_int;
    fn db_sethook(L: *mut lua_State) -> libc::c_int;
}
